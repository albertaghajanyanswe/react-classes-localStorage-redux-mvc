...
class CreateMeetingView extends Component {
  ...
  fixDate = (values) => {
    ...
    if (values.time) {
      values.time[0] = moment(values.time[0]).format("HH:mm");
      values.time[1] = moment(values.time[1]).format("HH:mm");
    }
  }
  ...
  updateMeeting = (values) => {
    ...
    //   return i.id === this.props.editedMeeting.id
    // })
    console.log("EDIRED ...........", this.props.editedMeeting)
    const originalObj = {...this.props.editedMeeting, ...values}
    this.fixDate({...this.props.editedMeeting, ...values});

    this.props.editMeeting({...this.props.editedMeeting, ...values});
    this.setState({editedData: {}, loading: true});
    this.props.handleOk();
  }
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    editMeeting(editedMeeting) {
      return dispatch(MeetingAction.editMeeting(editedMeeting));
    }
  }
}
