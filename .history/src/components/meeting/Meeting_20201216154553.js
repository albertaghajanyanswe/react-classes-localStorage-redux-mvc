         ...
         <Row gutter={16}>
             {meetings && meetings.map(i => {
               if (i.ownerId === user.id || i.membersId.includes(user.id)) {
                 return <MeetingCard 
                          item={i} key={i.id} setNeedToRender={this.setNeedToRender} needToRender={needToRender} setIsEditMode={this.setIsEditMode} setEditedMeeting={this.setEditedMeeting} setShowCreateModal={this.setShowCreateModal}/>
               }
           })}
         </Row>
       </div>
